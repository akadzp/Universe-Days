...
export function applyCharacterIndicatorEffectWorkflow(payload: ApplyCharacterIndicatorEffectPayload): WorkflowDefinition {
  return {
    ...
    steps: [{
      ...
      executor: (ctx, tx) => {
        ...
        if (!character) return failure('CHARACTER_NOT_FOUND', `Character '${id}' was not found.`);

        const aggregate = {
          character,
          behaviors: Object.freeze(Object.fromEntries(
            (character.behaviorReferences ?? [])
              .filter(ref => ctx.universeSnapshot.behaviors[ref])
              .map(ref => [ref, ctx.universeSnapshot.behaviors[ref]])
          )),
          styles: Object.freeze(Object.fromEntries(
        ...
        if (!validation.valid) return failure('INVALID_CHARACTER_AGGREGATE', validation.issues.map(issue => issue.message).join('; '));

        const mutation = CharacterAggregateLifecycle.applyIndicatorEffect(aggregate, {
        ...
        if (!mutation.valid || !mutation.aggregate) {
          return failure('CHARACTER_INDICATOR_MUTATION_REJECTED', mutation.issues.map(issue => issue.message).join('; '));
        }
