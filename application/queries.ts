...
import type { ApplicationQueryContext, ApplicationQueryType, CharacterSummaryDTO, DailyCycleStatusDTO, UniverseSummaryDTO } from './contracts.ts';
import { ApplicationAuthorizer } from './authorization.ts';

export class ApplicationQueryService {
  public constructor(private readonly executionEngine: PocerExecutionEngine, private readonly authorizer: ApplicationAuthorizer = new ApplicationAuthorizer()) {}
  ...
  public characterSummary(context: ApplicationQueryContext, characterId: string): CharacterSummaryDTO | null {
  ...
  public listCharacters(context: ApplicationQueryContext): readonly CharacterSummaryDTO[] {
    ...
    const universe = this.snapshot(context);
    return Object.freeze(Object.values(universe.characters).map(character => this.toCharacterSummary(character)));
  }
  ...
  public dailyCycleStatus(context: ApplicationQueryContext): DailyCycleStatusDTO {
    ...
    return Object.freeze({
      ...
      activeProcessCount: Object.values(snapshot.processes).filter(process => process.currentStatus === 'ACTIVE').length,
    });
  }
  ...
  private execute(query: UniverseQuery): { data: unknown } {
    ...
    if (!result.success || !result.data) throw new Error(result.message ?? `Application query '${query.queryId}' failed.`);
    return result.data;
  }
  ...
  private toCharacterSummary(character: CharacterEntity): CharacterSummaryDTO {
    const indicatorKeys = character.indicators
      ? Object.entries(character.indicators)
          ...
          .flatMap(([groupKey, group]) => groupKey === 'history' ? [] : Object.keys(group as object))
      : [];
    return Object.freeze({
