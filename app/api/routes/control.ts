...
controlRouter.get('/universe/daily-context', (_req, res) => {
  try {
    ...
    const dateStr = u.temporalContext.currentUniverseDate;
    if (!dateStr) return res.status(409).json({ error: 'TEMPORAL_CONTEXT_UNRESOLVED' });
...
function handleAdvanceDay(req: any, res: any) {
  try {
    ...
    if (!currentDateValue) return res.status(409).json({ error: 'TEMPORAL_CONTEXT_UNRESOLVED', message: 'Tanggal semesta belum ditentukan.' });
    const currentDate = new Date(currentDateValue);
    if (Number.isNaN(currentDate.getTime())) return res.status(409).json({ error: 'TEMPORAL_CONTEXT_INVALID', message: 'Tanggal semesta tidak valid.' });
