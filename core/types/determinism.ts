/**
 * Shared deterministic serialization and identity helpers.
 * This module is dependency-light so architecture layers do not depend on Engine.
 */
...
export function deterministicId(prefix: string, ...parts: unknown[]): string {
  return `${prefix}_${fnv1a32(parts.map(stableSerialize).join('::'))}`;
}
