/** Phase 29/34 — deterministic real production runner. */
...
export class ProductionRunner {
  ...
  public constructor(private readonly contextCompiler: ProductionContextCompiler, private readonly ai: AIProductionService, private readonly semanticCache: SemanticCache<any>, private readonly costController: CostController, private readonly productionStore: FileProductionStore) {}
  ...
  public async run(input: ProductionRunInput = {}): Promise<ProductionRunResult> {
    ...
    if (!universe) {
      ...
      await this.productionStore.save(result); return result;
    }
    ...
    if (!input.bypassCache) {
      ...
      if (cached) {
        ...
        return Object.freeze({ runId, status: 'CACHED', universeId, universeScope, purpose, timestamp, inputTokens: 0, outputTokens: 0, cost: 0, contextFingerprint, cached: true, output: cached.value });
      }
    }
