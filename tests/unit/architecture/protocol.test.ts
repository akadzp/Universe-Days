...
import { MessageType, ProtocolStatus, createProtocolMessage, validateProtocolMessage } from '../../../core/architecture/protocol.ts';
...
describe('Architecture Core - Protocol', () => {
  it('1. Valid READ message accepted', () => {
    const msg = createProtocolMessage(MessageType.READ, makeSystemID('CONSUMER_A'), makeSystemID('TEMPORAL_SYSTEM'), { query: 'current_epoch' }, { requestId: makeRequestID('REQ_PROTOCOL_READ_001'), timestamp: 0 });
  ...
  it('2. Valid REQUEST message accepted', () => {
    const msg = createProtocolMessage(MessageType.REQUEST, makeSystemID('DAILY_UNIVERSE_SYSTEM'), makeSystemID('CHARACTER_SYSTEM'), { intent: 'request_vitality_drain', amount: 10 }, { requestId: makeRequestID('REQ_PROTOCOL_REQUEST_001'), timestamp: 0 });
    const validation = validateProtocolMessage(msg);
    assert.strictEqual(validation.status, ResultStatus.SUCCESS);
    assert.ok(validation.data);
  ...
  it('3. Invalid message type rejected', () => {
    const invalidMsg = { requestId: makeRequestID('REQ-001'), type: 'TELEPORT_MAGIC', source: makeSystemID('SYSTEM_A'), target: makeSystemID('SYSTEM_B'), status: ProtocolStatus.PENDING, timestamp: 0 };
  ...
  it('4. Missing source rejected', () => {
    const missingSourceMsg = { requestId: makeRequestID('REQ-002'), type: MessageType.READ, target: makeSystemID('TEMPORAL_SYSTEM'), status: ProtocolStatus.PENDING, timestamp: 0 };
  ...
  it('5. Missing target rejected', () => {
    const missingTargetMsg = { requestId: makeRequestID('REQ-003'), type: MessageType.REQUEST, source: makeSystemID('DAILY_UNIVERSE_SYSTEM'), status: ProtocolStatus.PENDING, timestamp: 0 };
